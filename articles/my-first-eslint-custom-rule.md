...
アプリチームでは現在フロントエンドの刷新を行っています。
刷新中のコード内には、string 型の case ラベルに基づいて分岐する switch 文が複数あります。
しかし同じ条件で分岐しているのにも関わらず 各 switch 文で case ラベルの順序が一貫していないため、「あっちの switch 文ではこのラベルは上の方にあったけどこっちの switch 文では下の方にある」ということがあり、目的のラベルを見つけるのに少し手間がかかるという問題がありました。
そこで、「switch 文の中の case ラベルをアルファベット順に強制するような ESLint のカスタムルールを作れば良いのでは？」と思い立ったのがきっかけでした。

# 実際にやったことの流れ

ここからは実際にどのように ESLint のカスタムルールを作って行ったかを書いていこうと思います。
...
- [typescript-eslint/switch-exhaustiveness-check](https://typescript-eslint.io/rules/switch-exhaustiveness-check/)
  - union 型を網羅するとか、default 句を強制するかどうかなどについてのルール
...
lint を走らせた時に表示されるメッセージやオプションについては、今回作りたいルールと同じルールがすでにあるか調べた時に出てきた、ESLint 標準のルールである[sort-keys](https://eslint.org/docs/latest/rules/sort-keys)を参考にすることにしました。
というのも、利用者からすると ESLint 標準のルールと同じようなオプションの渡し方ができたり、同じようなエラーメッセージを得られたりした方が、学習コストが低く使い勝手が良いのではないかと考えたからです。
...
＜最初はアルファベット順にすることだけ考えていたけど sort-keys にあるようなオプション（箇条書き）を渡せるようにすることにしました＞
カスタムルールを作り始めようとした時は、単純に「case ラベルをアルファベットの昇順にすることを強制する」だけルールを考えていましたが、sort-keys のオプションを見てみて、
...
元々このルールを作ったきっかけは、業務のコードでの辛みがあったからだったので、早速いつも自分が業務で読み書きしているアプリ領域のディレクトリにインストールして PR を作成しました。
...
スピーディに自分の作ったルールが適用されたのは、kintone 新機能開発チームにこのような体制があったからこそなのかなと感じました。
...
パッケージ公開後、アプリチーム内で週１で行なっている LT 会でカスタムルールを作った話をしたところ、たまたまフロントエンドエキスパートの方が聴きに来てくださっていて、そこで教えてもらった eslint-plugin-eslint-plugin というプラグインを今回作った eslint-plugin-switch-order-case に導入してみました。
